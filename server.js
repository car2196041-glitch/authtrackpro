...
app.post('/authorizations', async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `INSERT INTO authorizations
      (patient_name, dob, payer, procedure_name, cpt_code, status, priority, submitted_date, auth_number, notes)
...
app.post("/authorizations", async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `INSERT INTO authorizations 
      (patient_name, payer, procedure_name, cpt_code, status, priority)
      VALUES ($1, $2, $3, $4, $5, $6)
      RETURNING *`,
      [patient_name, payer, procedure_name, cpt_code, status, priority]
    );
    ...
    console.error("POST /authorizations error:", err);
